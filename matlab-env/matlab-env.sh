...
if [[ -f "$FREETYPE" ]]; then
    echo "MATLAB's version of libfreetype is present! This causes issues with MATLAB Addons!\nPlease exclude or delete from ./matlab/bin/glnxa64"
    exit 1
fi

LIBSTDCXX=./matlab/sys/os/glnxa64/libstdc++.so.6
if [[ -f "$FREETYPE" ]]; then
    echo "MATLAB's version of libstdc++ is present! This causes issues with MATLAB OpenGL Acceleration!\nPlease exclude or delete from ./matlab/sys/os/glnxa64"
    exit 1
fi
